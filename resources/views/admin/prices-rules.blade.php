        ...
        <div class="admin-main">
            ...
            <div class="block-country-new">
                <div class="create-buton" >
                    <button type="submit" class="btn btn-primary" id="create-price">
                        <i class="fa fa-plus"> {{  __('language.price')}}</i>

                    </button>
                </div>
                <div class="inactive-countries-checkbox inactive-buton">
                    <label class="container-checkbox">{{  __('language.inactive')}} {{  __('language.prices')}}
                        <input id="inActivePrices" type="checkbox" {{($data['allPrices'] == true)? 'checked=checked' :''}}>
            ...
            <section style="margin: 10px;">
                <fieldset style="min-height:100px;">
                    <legend><b> Легенда </b> </legend>
                    <div class="input-group mb-1">
                        <div class="input-group-prepend">
                            <span class="input-group-text" style="background: #00fa9a6b" id="inputGroup-sizing-default"></span>
                        </div>
                        <input type="text" style="margin-right: 15px;" class="form-control" id="info-ac" aria-label="Sizing example input" value="Цената е с активирано правило" aria-describedby="inputGroup-sizing-default" disabled>
                    </div>
                    <div class="input-group mb-1">
                        <div class="input-group-prepend">
                            <span class="input-group-text" style="background: #ff9e9eb0" id="inputGroup-sizing-default"></span>
                        </div>
                        <input type="text" class="form-control" style="margin-right: 15px;" id="info-ac" aria-label="Sizing example input" value="Цената е неактивна" aria-describedby="inputGroup-sizing-default" disabled>
                    </div>
                    <div class="input-group mb-1">
                        <div class="input-group-prepend">
                            <span class="input-group-text" style="background: #FFF" id="inputGroup-sizing-default"></span>
                        </div>
                        <input type="text" class="form-control" style="margin-right: 15px;" id="info-ac" aria-label="Sizing example input" value="Цената е свободна" aria-describedby="inputGroup-sizing-default" disabled>
                    </div>
            ...
            <div class="test">
                <h2>{{ __('language.prices') }}</h2>
                <ul class="responsive-table">
                    <li class="table-header">
                        <div class="col col-1">#</div>
                        <div class="col col-3" style="flex-basis: 23%;">{{ __('language.price')}}</div>
                    ...
                    @foreach ($data['prices'] as $price)
                        ...
                        $color = '#FFF';
                            ...
                            if($price->isActive == 1 && $price->isUsed == 1){
                                $color = '#00fa9a6b';

                            }
                        ?>
                        <li class="table-row item_{{$index}}" style="background-color: {{$color}}">
                            <div class="col col-1 cell iteration" data-label="#">{{$index}}</div>
                            <div class="col col-3 cell name" style="flex-basis: 23%;" data-label="{{ __('language.name')}}">{{$price->Name}}</div>
                            @if($price->isActive == 1)
                                <div class="col col-1 cell status" data-content="{{$price->isActive}}" data-label="{{ __('language.status')}}">{{ __('language.active') }}</div>
                            @else
                                <div class="col col-1 cell status" data-content="{{$price->isActive}}" data-label="{{ __('language.status')}}">{{ __('language.inactive') }}</div>
                            @endif

                            <div class="col col-3 cell">
                                <button type="button" class="btn btn-info responsive-btn-left"  id="edit-price" data-item-id="{{$price->id}}">
                                    <i class="fa fa-edit"></i></button>
                                {{--<button type="button" class="btn btn-info responsive-btn-right" id="delete-country" data-item-id="{{$countries->id}}">--}}
            ...
            <div class="block-price-rule-new">
                <div class="create-buton">
                    <button type="submit" style="margin-top: 14px;" class="btn btn-primary" id="create-price-rule">
                        <i class="fa fa-plus"> {{  __('language.price-rule')}}</i>

                    </button>
                </div>
                <div class="inactive-rule-checkbox inactive-buton">
                    <label class="container-checkbox">{{  __('language.inactive')}} Rules
                        <input id="inActiveRules" type="checkbox" {{($data['allPriceRules'] == true)? 'checked=checked' :''}}>
                        <span class="checkmark-checkbox"></span>
                    </label>
            ...
            <div class="cities">
                <h2>{{  __('language.price-rules')}}</h2>
                <ul class="responsive-table">
                    <li class="table-header"   style="font-size: 13px; text-transform: none;">
                        <div class="col col-1 small-column">#</div>
                        <div class="col col-3 small-column">{{ __('language.price')}} {{ __('language.name')}}</div>
                        {{--<div class="col col-2 small-column">{{ __('language.description')}}</div>--}}
                        <div class="col col-2 small-column" style="flex-basis: 11%;">{{ __('language.date_from')}}</div>
                        <div class="col col-2 small-column" style="flex-basis: 11%;">{{ __('language.date_to')}}</div>
                        ...
                        <div class="col col-1 small-column">Коли</div>
                        <div class="col col-3"></div>
                    </li>
                    <?php $index = 0; ?>
                    @foreach ($data['price_rules'] as $rule)
                        <?php $index++; ?>
                        <li class="table-row item_{{$index}}" style="font-size: 12px; {{($rule->isActive == 0)? 'background-color: #ff9e9efc':  ''}}">
                            <div class="col col-1 cell small-column iteration" data-label="#">{{$index}}</div>
                            <div class="col col-3 cell small-column name" data-content="{{$rule->price_id}}" data-label="{{ __('language.name')}}">{{$rule->Name}}</div>
                            {{--<div class="col col-2 cell medium-column description" data-label="{{ __('language.description')}}">{{$rule->Description}}</div>--}}
                            <div class="col col-2 cell small-column date_from" style="flex-basis: 11%;" data-label="{{ __('language.date_from')}}">{{$rule->start_date}}</div>
                            <div class="col col-2 cell small-column date_to" style="flex-basis: 11%;" data-label="{{ __('language.date_to')}}">{{$rule->end_date}}</div>
                            <div class="col col-1 cell small-column price hidden-column" data-label="{{ __('language.price')}}">{{$rule->RulePrice}}</div>
                            <div class="col col-1 cell small-column price1 hidden-column" data-label="{{ __('language.price')}}">{{$rule->price_day}}</div>
                            <div class="col col-1 cell small-column price2_4 hidden-column" data-label="{{ __('language.price')}}">{{$rule->price_2_4_days}}</div>
                            <div class="col col-1 cell small-column price5_7 hidden-column" data-label="{{ __('language.price')}}">{{$rule->price_5_7_days}}</div>
                            <div class="col col-1 cell small-column price8_15 hidden-column" data-label="{{ __('language.price')}}">{{$rule->price_8_15_days}}</div>
                            <div class="col col-1 cell small-column price16_30 hidden-column" data-label="{{ __('language.price')}}">{{$rule->price_16_30_days}}</div>
                            <div class="col col-1 cell small-column price_weekend hidden-column" data-label="{{ __('language.price')}}">{{$rule->price_weekend}}</div>
                            <div class="col col-1 cell small-column price31 hidden-column" data-label="{{ __('language.price')}}">{{$rule->price_31_days}}</div>
                            <div class="col col-1 cell small-column discount hidden-column" data-label="{{ __('language.discount')}}">{{$rule->discount_percent}}</div>
                            <div class="col col-1 cell small-column model" data-content="{{$rule->model_id}}" data-label="{{ __('language.model')}}">{{$rule->ModelName}}</div>
                            <div class="col col-1 cell small-column year" data-content="{{$rule->ac}}" data-label="{{ __('language.year')}}">{{$rule->ModelYear}}</div>


                        @if($rule->gear_type == 1)
                                <div class="col col-1 cell small-column gear"  data-content="{{$rule->gear_type}}" data-label="{{ __('language.gear')}}">A</div>
                            @else
                                <div class="col col-1 cell small-column gear"   data-content="{{$rule->gear_type}}" data-label="{{ __('language.gear')}}">M</div>
                            @endif
                            @if($rule->isActive == 1)
                                <div class="col col-1 cell small-column status" data-content="{{$rule->isActive}}" data-label="{{ __('language.status')}}">{{ __('language.active') }}</div>
                            @else
                                <div class="col col-1 cell small-column status" data-content="{{$rule->isActive}}" data-label="{{ __('language.status')}}">{{ __('language.inactive') }}</div>
                            @endif
                            <div class="col col-1 cell small-column used" data-label="Коли">{{$rule->count_cars_used}}</div>


                            <div class="col col-3 cell">
                                <button type="button" class="btn btn-info responsive-btn-left"  id="edit-price-rule" data-item-id="{{$rule->id}}">
                                    <i class="fa fa-edit"></i>
                                </button>
                                <button type="button" class="btn btn-info responsive-btn-right" id="info-price-rule" data-item-id="{{$rule->id}}">
